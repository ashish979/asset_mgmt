  ...
  module Audit
    ...
    module ClassMethods
      def setup_audit
        ...
        before_create :set_version_number, :set_audit_admin
      ...
      def as_admin(admin, &block)
        Thread.current[:audited_admin] = admin
        yield
      ensure
        Thread.current[:audited_admin] = nil
      end
    ...
    def set_audit_admin
      self.admin = Thread.current[:audited_admin] if Thread.current[:audited_admin]
      self.remote_address = Thread.current[:ip]
